...
import type { FunnelHealthData } from "@/lib/mock-client-recruiter-home"
...
/** Client-recruiter analog of the recruiter home page's AgentHealthCard — a compact funnel diagnostic layer, not a full analytics module. */
export function FunnelHealthCard({ data }: { data: FunnelHealthData }) {
  return (
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto no-scrollbar">
        <div>
          <p className="text-3xl font-semibold tracking-tight text-foreground">
            {data.submissionToInterviewPct}%
          </p>
          <p className="text-xs text-muted-foreground">submission → interview conversion</p>
        </div>
