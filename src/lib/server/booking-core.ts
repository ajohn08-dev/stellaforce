...
/**
 * The candidate's side of booking — the whole of it.
 *
 * This module is the single implementation. Two transports call it and neither
 * owns any logic: `src/app/book/actions.ts` (Server Actions, for the React
 * component) and `src/app/api/book/[token]/*` (HTTP routes, for anything that
 * isn't a React component — and for the client, which uses them so that the
 * cache, rate-limit and idempotency headers apply). Putting the rules in either
 * transport would mean two answers to "may this token do this", and the wrong
...
/**
 * Just the parts that go stale: the live grid, whether start-now is possible
 * right now, and how long a lease lasts.
 *
 * Deliberately a strict subset of `BookingView` with **no names on it**. The
 * page already rendered the job title and the candidate's first name server-side
 * from a token that was valid then; a polling endpoint that re-sent them would
 * put identifying strings on the wire on a timer, for no benefit.
 */
export type AvailabilityView = {
  ...
  holdSeconds: number
  operatingTimezone: string
  /** The link is spent. The client should stop polling and show the confirmation. */
  booked: boolean
}
...
export type ConfirmResult =
  | { ok: true; scheduledAt: string; startNow: boolean; replayed: boolean }
  | { ok: false; message: string; reasonCode?: SchedulingReasonCode }
...
export async function loadBooking(token: string): Promise<BookingView | null> {
  ...
  if (request.status === "booked" && request.interview_id) {
    const interview = await loadBookedInterview(admin, request.interview_id)
  ...
  if (!isOpenForBooking(request)) return null

  const context = await loadContext(admin, request)
  if (!context) return null

  const live = await computeAvailability(admin, request)

  return {
    ...context,
    slots: live.slots,
    canStartNow: live.canStartNow,
    booked: null,
  }
}

/**
 * The live grid, for the availability route.
 *
 * Returns null for every unusable-link condition — expired, cancelled, unknown —
 * so the caller has nothing to tell apart and cannot accidentally leak the
 * difference through a status code.
 */
export async function loadAvailability(token: string): Promise<AvailabilityView | null> {
  const admin = createAdminClient()
  const request = await resolveBookingToken(admin, token)
  if (!request) return null
 ...
 *
 * **Idempotent by construction.** A replay finds `status = 'booked'`, gets
 * `CANDIDATE_ALREADY_BOOKED` back with the existing `interview_id`, and is
 * answered with that booking rather than an error. Which is the honest answer:
 * the caller asked for this token to be booked, and it is. `replayed` says so,
 * so a UI can word the confirmation differently if it wants to; nothing depends
 * on it.
 */
export async function confirmBooking(input: {
  ...
  if (data.reason_code) {
    ...
    // The durable half of idempotency. The RPC returns the interview it already
    // created, so a double-click, a retried request, or the same link reopened
    // next week all resolve to the one booking that exists.
    if (code === "CANDIDATE_ALREADY_BOOKED" && data.interview_id) {
      const booked = await loadBookedInterview(admin, data.interview_id)
      if (booked) {
        return {
          ok: true,
...
/** Is this link still allowed to book something? */
function isOpenForBooking(request: ResolvedBookingRequest): boolean {
  if (request.status !== "pending" && request.status !== "sent") return false
  return new Date(request.token_expires_at).getTime() > Date.now()
}

/** The booking of record, reduced to what a candidate may be shown. */
async function loadBookedInterview(
  admin: ReturnType<typeof createAdminClient>,
  interviewId: string
): Promise<{ scheduledAt: string; timezone: string | null } | null> {
  const { data } = await admin
    ...
    .maybeSingle()
  if (!data) return null
  return { scheduledAt: data.scheduled_at, timezone: data.candidate_timezone }
}

/** The live grid — shared by the page load and the availability route. */
async function computeAvailability(
  admin: ReturnType<typeof createAdminClient>,
  request: ResolvedBookingRequest
): Promise<{ slots: SlotOption[]; canStartNow: boolean }> {
